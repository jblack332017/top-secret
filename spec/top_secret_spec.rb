...
RSpec.describe TopSecret do
  ...
  it 'returns large size' do
    expect(TopSecret::Scrape.positive(large_url, 10, 100).length).to eql(100)
  end
...
EOT
  ...
  it 'accepts invalid url' do
    expect { TopSecret::Scrape.positive(invalid_url, 5, 3) }.to_not raise_error
  end
end
